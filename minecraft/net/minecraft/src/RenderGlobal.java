...
//            Main, ICamera, RenderHelper, TileEntity, 
//            WorldProvider, Vec3D, EntityPlayerSP, RenderSorter, 
//            MovingObjectPosition, EntityPlayer, ItemStack, EnumMovingObjectType, 
//            AxisAlignedBB, GuiIngame, SoundManager, EntityHugeExplodeFX, 
//            EntityFX, EffectRenderer, EntityLargeExplodeFX, EntityBubbleFX, 
//            EntitySuspendFX, EntityAuraFX, EntityCritFX, EntitySmokeFX, 
//            EntitySpellParticleFX, EntityNoteFX, EntityPortalFX, EntityEnchantmentTableParticleFX, 
//            EntityExplodeFX, EntityFlameFX, EntityLavaFX, EntityFootStepFX, 
//            EntitySplashFX, EntityCloudFX, EntityReddustFX, EntityBreakingFX, 
//            Item, EntityDropParticleFX, Material, EntitySnowShovelFX, 
//            EntityHeartFX, EntityDiggingFX, ImageBufferDownload, ItemPotion, 
//            StepSound, ItemRecord
    ...
    implements IWorldAccess
{
    ...
    {
    	if(Main.ESP || Main.tracer){
    		TracerLine();
    		TracerLineFriend();
    	}
    	if(Main.tracerdiamond){
    		tracerdiamond();
    	}
        ...
        for(int j = 0; j < list.size(); j++)
        {
            ...
            if(Main.norender && !(entity1 instanceof EntityLiving) || !entity1.isInRangeToRenderVec3D(vec3d) || !entity1.ignoreFrustumCheck && !icamera.isBoundingBoxInFrustum(entity1.boundingBox) || entity1 == mc.renderViewEntity && mc.gameSettings.thirdPersonView == 0 && !mc.renderViewEntity.isPlayerSleeping())
            {
                continue;
            }
    ...
    public void tracerdiamond() {
    	GameSettings.viewBobbing = false;
        int length = 50;//how big you want your search to be for example this will search 50 blocks in all directions not recommended to do more then 70
        for(int x = -length; x < length + 1; x++){//dis shit loops the blocks
        for(int z = -length; z < length + 1; z++){//more looping of the blocks
        for(int y = -length; y < length + 1; y++){//more shit....
        int i = (int) mc.thePlayer.posX + x;//more shit for finding your position
        int j = (int) mc.thePlayer.posY + y;//it's petty much nuker code edited a bit
        int k = (int) mc.thePlayer.posZ + z;
        Block block = Block.blocksList[mc.theWorld.getBlockId( x+i, y+j, z+k)];//get's the block Id
        if((mc.theWorld.getBlockId(x+i, y+j, z+k) == 54))//if the block ID is equal to this than the shit under it happens
        {
               
                int X = x+i;
                int Y =y+j;
                int Z = z+k;
        double posX = X - mc.thePlayer.posX;//X  position of block
        double posY = Y - mc.thePlayer.posY;//y
        double posZ = Z - mc.thePlayer.posZ;//z
        GL11.glPushMatrix();
        GL11.glColor3f(0,255,0);//can change this, it's the Color RGB
        GL11.glDisable(3553);
        GL11.glDisable(2896 /*GL_LIGHTING*/);
        GL11.glDepthMask(false);
        GL11.glLineWidth(0.8F);//change this to your liking
        GL11.glBlendFunc(770, 771);
        GL11.glEnable(GL11.GL_LINE_SMOOTH);
        GL11.glBegin(GL11.GL_LINE_LOOP);
        GL11.glVertex2d(0D, 0D);//first vertex (center of screen)
        GL11.glVertex3d(posX,  posY, posZ);//second vertex for the block
        GL11.glEnd();
    ...
    public void TracerLineFriend() {
    	GameSettings.viewBobbing = false;
    	for(int p =0; p < mc.theWorld.playerEntities.size(); p++) 
    	{
    		EntityPlayer player = (EntityPlayer) mc.theWorld.playerEntities.get(p);
    		if(!player.username.equals("2dieis2live") || !player.username.equals("Captain_Derple") || !player.username.equals("RANDOM1ZA") ||!player.username.equals("simran123")){
    			double X =player.boundingBox.minX - mc.thePlayer.boundingBox.minX;
    			double Y =player.boundingBox.maxY - mc.thePlayer.boundingBox.maxY;
    			double Z =player.boundingBox.minZ - mc.thePlayer.boundingBox.minZ;
    		
    		GL11.glPushMatrix();
	        GL11.glColor4f(0.0F, 8.0F, 70.0F, 1.0F); // LINE COLOR
	        GL11.glLineWidth(1.5F); //LINE WIDTH
	        GL11.glDisable(3553);
	        GL11.glDisable(2896 /*GL_LIGHTING*/);
	        GL11.glBlendFunc(770, 771);
	        GL11.glEnable(GL11.GL_LINE_SMOOTH);
	        ...
	        GL11.glVertex3d(X, Y, Z); //ENTITIES X, Y & Z
	        GL11.glEnd();
	        GL11.glDisable(GL11.GL_LINE_SMOOTH);
	        GL11.glEnable(3553);
	        GL11.glEnable(2896 /*GL_LIGHTING*/);
    	...
    	for(int p =0; p < mc.theWorld.playerEntities.size(); p++) 
    	{
    		...
    		EntityPlayer player = (EntityPlayer) mc.theWorld.playerEntities.get(p);
    			...
    			double Z =player.boundingBox.minZ - mc.thePlayer.boundingBox.minZ;
    			  ...
    			  GL11.glPushMatrix();
    		        GL11.glColor4f(0.0F, 1.0F, 1.0F, 1.0F); // LINE COLOR
    		        GL11.glLineWidth(1.5F); //LINE WIDTH
    		        GL11.glDisable(3553);
    		        GL11.glDisable(2896 /*GL_LIGHTING*/);
    		        GL11.glBlendFunc(770, 771);
